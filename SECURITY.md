...
| ☑ | 🟡 Medium | Agent filesystem scope | Agent had no explicit directory scope, could traverse arbitrary paths | `--add-dir` restricts agent file-access to `~/workspace`, `~/workspace.worktrees`, and `~/.copilot`; the same dirs are written to `trustedFolders` in `config.json` at startup. Both are driven by `COPILOT_ADD_DIRS`, which **extends** the defaults — it does not replace them. |
